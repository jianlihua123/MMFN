...
class SRSolver(BaseSolver):
    ...
    def tv_loss(self, img):
        ...
        loss = (w_variance + h_variance)/ height/width/chan
        return loss
    ...
    def train_step(self):
        ...
        for i in range(self.split_batch):
            ...
            split_PAN = self.PAN.narrow(0, i * sub_batch_size, sub_batch_size)
            

######## l1 loss

            # if self.use_cl:
            #     outputs = self.model(split_LR, split_PAN)
            ...
            # else:
            #     output = self.model(split_LR, split_PAN)
            #     loss_sbatch = self.criterion_pix(output, split_HR)

######### l1 + TV loss

            # split_PAN_expand = split_PAN.expand(-1,4,-1,-1) ##
            # print(split_PAN_expand.shape) ##
            # output = self.model(split_LR, split_PAN)
            # if type(output) is tuple:
            #     num_output = len(output) - 1

            #     loss_sbatch = torch.tensor([0.], requires_grad=True)
            #     if self.use_gpu:
            #         loss_sbatch = loss_sbatch.cuda()

            #     # loss_sbatch += self.criterion_pix(output[0], split_HR) + self.tv_loss(output[-1] - split_PAN_expand)
            #     loss_sbatch += self.criterion_pix(output[0], split_HR)

######## multi-l1-loss

            output = self.model(split_LR, split_PAN)
            if type(output) is tuple:
                # num_output = len(output) - 1  # error
                num_output = len(output)
                # print(len(output))

                for i in range(num_output):

                    # print(self.cl_weights[i])
                    loss_sbatch += self.criterion_pix(output[i], split_HR)*self.cl_weights[i]
                    split_HR = nn.functional.interpolate(split_HR, scale_factor=0.5,
                                                         mode='bilinear', align_corners=False)
                loss_sbatch /= num_output
            else:
                loss_sbatch += self.criterion_pix(output[i], split_HR)
            #     print(loss_batch)
...
######## multi-l1+TV-loss
            ...
            #         loss_sbatch += self.criterion_pix(output[i], split_HR)*self.cl_weights[i] + self.tv_loss(output[i] - split_PAN_expand)*self.cl_weights[i]*0.001
            #         split_HR = nn.functional.interpolate(split_HR, scale_factor=0.5,
            #                                              mode='bilinear', align_corners=False)
            #         split_PAN_expand = nn.functional.interpolate(split_PAN_expand, scale_factor=0.5,
            #                                              mode='bilinear', align_corners=False)
            #     loss_sbatch /= num_output
            # else:
            #     loss_sbatch += self.criterion_pix(output[i], split_HR)
...
############
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if self.self_ensemble and not self.is_train:
                SR = self._forward_x8(self.LR, self.PAN, forward_func)
            else:
    ...
    def _forward_x8(self, x, pan, forward_function):
        """
        self ensemble
        """
        ...
        for tf in 'v', 'h', 't':
            ...
            pan_list.extend([_transform(p, tf) for p in pan_list])

        sr_list = []
        #        for aug in lr_list:
        for i in range(len(lr_list)):
            sr = forward_function(lr_list[i], pan_list[i])
            if isinstance(sr, list):
                sr_list.append(sr[-1])
            else:
                sr_list.append(sr)
    ...
    def _overlap_crop_forward(self, x, pan, shave=10, min_size=100000, bic=None):
        ...
        if w_size * h_size < min_size:
            ...
            for i in range(0, 4, n_GPUs):
                ...
                sr_batch_temp = self.model(lr_batch, pan_batch)
