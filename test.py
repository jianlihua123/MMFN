...
#    opt = option.parse(parser.parse_args().opt)
    opt = option.parse('./options/test/test_MFPS_example.json')
    ...
    for bm, test_loader in zip(bm_names, test_loaders):
        ...
        for iter, batch in enumerate(test_loader):
            ...
            if need_HR:
                cc, ssim = util.calc_metrics(visuals['SR'], visuals['HR'], crop_border=scale)
                total_psnr.append(cc)
                total_ssim.append(ssim)
                path_list.append(os.path.basename(batch['HR_path'][0]).replace('HR', model_name))
                print("[%d/%d] %s || PSNR(dB)/SSIM: %.4f/%.4f || Timer: %.4f sec ." % (iter+1, len(test_loader),
        ...
        if need_HR:
            save_img_path = os.path.join('./results/'+degrad, model_name, bm, "x%d"%scale)
        else:
            save_img_path = os.path.join('./results/'+bm, model_name, "x%d"%scale)
        ...
        for img, name in zip(sr_list, path_list):
            img = util.Tensor2np([img], 2047)[0]
            np.save(os.path.join(save_img_path, name), img)
